...
class Profile extends CI_Controller {
    ...
    public function index($success = '0')
    {
        ...
        if($data['photo'])
            $data['photo'] = 'upload/profile/large'.$data['photo'];
    ...
    public function save()
    {
        ...
        $this->form_validation->set_rules('lab', 'Lab Group', 'trim|required');
        ...
        $this->session->set_userdata('lab', $sql_data['lab']);
    ...
    public function view($user_id)
    {
        ...
        if($data['user']['photo'])
            $data['user']['photo'] = 'upload/profile/large'.$data['user']['photo'];
