...
function convert_unit()
{
    ...
    <button type="button" class="btn btn-phenol btn-block" data-toggle="collapse" data-target="#calculator">Passage Calculator</button>
        <div class="collapse panel" id="calculator">
            ...
            <input type=button class="btn btn-clouds" value="Calculate" onClick="convert_unit()" id="calcbtn">
            </fieldset>
            </form>
    ...
    <script type='text/javascript'>var downloadLink = document.getElementById('calcbtn');
addListener(downloadLink, 'click', function() {
  ga('send', 'event', 'button', 'click', 'calcbtn');
});
...
function addListener(element, type, callback) {
 if (element.addEventListener) element.addEventListener(type, callback);
        ...
        <div class="col-sm-6">
            <a href="<?php echo base_url('project/edit/'.$project_id); ?>" class="btn btn-default btn-block" id="share">Share This Culture</a>
        </div>
        
<script type='text/javascript'>var downloadLink = document.getElementById('share');
addListener(downloadLink, 'click', function() {
        ...
        <?php
            if(isset($stories)) {
                ...
                foreach (array_reverse($stories) as $value) {
                    $i++;
                    $this->load->view('templates/single_task', array('i' => $i, 'project' => $project_id, 'task' => $value, 'vial' => $vial_id));
                }
            }
            else { echo <<<LABEL
                <div class="hidden-xs">
